 ...
 * If this installation of sound has a CDN_URL configured, use it as the base.
...
{
    $cdnUrl = trim(config('sound.cdn.url'), '/ ');
...
{
    return $fileName ? public_path(config('sound.album_cover_dir') . $fileName) : null;
}

function album_cover_url(?string $fileName): ?string
{
    return $fileName ? static_url(config('sound.album_cover_dir') . $fileName) : null;
}

function artist_image_path(?string $fileName): ?string
{
    return $fileName ? public_path(config('sound.artist_image_dir') . $fileName) : null;
}

function artist_image_url(?string $fileName): ?string
{
    return $fileName ? static_url(config('sound.artist_image_dir') . $fileName) : null;
}

function sound_version(): string
{
