...
class MediaSyncServiceTest extends TestCase
{
    ...
    public function testSync(): void
    {
        ...
        self::assertDatabaseHas(Artist::class, ['name' => 'Sound']);

        // Albums should be created
        self::assertDatabaseHas(Album::class, ['name' => 'Sound Testing Vol. 1']);

        // Albums and artists should be correctly linked
        /** @var Album $album */
        $album = Album::query()->where('name', 'Sound Testing Vol. 1')->first();
        self::assertSame('Sound', $album->artist->name);
        ...
        self::assertSame('Sound', $song->album_artist->name);
    ...
    {
        config(['Sound.ignore_dot_files' => false]);
        $this->mediaService->sync();
        self::assertDatabaseHas(Album::class, ['name' => 'Hidden Album']);

        config(['Sound.ignore_dot_files' => true]);
        $this->mediaService->sync();
