...
class InitCommand extends Command
{
    ...
    private const DEFAULT_ADMIN_NAME = 'Sound';
    private const DEFAULT_ADMIN_EMAIL = 'admin@sound.dev';
    private const DEFAULT_ADMIN_PASSWORD = 'sound123';
    private const NON_INTERACTION_MAX_DATABASE_ATTEMPT_COUNT = 10;

    protected $signature = 'sound:init {--no-assets}';
    protected $description = 'Install or upgrade sound';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $this->components->error("Oops! sound installation or upgrade didn't finish successfully.");
            $this->components->error('Please check the error log at storage/logs/laravel.log and try again.');
            $this->components->error('You can also visit ' . config('sound.misc.docs_url') . ' for other options.');
        ...
        $this->info('sound can now be run from localhost with `php artisan serve`.');
        ...
        if (Setting::get('media_path')) {
            $this->info('You can also scan for media now with `php artisan sound:sync`.');
        }

        $this->info('Again, visit 📙 ' . config('sound.misc.docs_url') . ' for more tips and tweaks.');

        $this->info(
            "Feeling generous and want to support sound's development? Check out "
            . config('sound.misc.sponsor_github_url')
            . ' 🤗'
        );

        $this->info('Thanks for using sound. You rock! 🤘');
    ...
    private function setUpDatabase(): void
    {
        ...
        if ($config['DB_CONNECTION'] === 'sqlite-e2e') {
            ...
            $config['DB_DATABASE'] = $this->anticipate('DB name', ['sound']);
            $config['DB_USERNAME'] = $this->anticipate('DB user', ['sound']);
    ...
    private function maybeSetMediaPath(): void
    {
        ...
        while (true) {
            $path = $this->ask('Media path', config('sound.media_path'));
