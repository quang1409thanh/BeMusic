...
class MediaSyncService
{
    ...
    public function gatherFiles(string $path): array
    {
        return iterator_to_array(
            $this->finder->create()
                ...
                ->ignoreDotFiles((bool) config('sound.ignore_dot_files')) // https://github.com/sound/sound/issues/450
                ->files()
                ->followLinks()
    ...
    private function setSystemRequirements(): void
    {
        if (!app()->runningInConsole()) {
            set_time_limit(config('sound.sync.timeout'));
        }

        if (config('sound.memory_limit')) {
            ini_set('memory_limit', config('sound.memory_limit') . 'M');
        }
    }
